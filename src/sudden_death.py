...
class SuddenDeath:
    ...
    def draw_spikes(self, surface, sprites, time):
        ...
        for y in range(0, self.level_height, 32):
            surface.blit(sprites[1], (time - 105, y))
            surface.blit(flipped_img, (self.level_with - time + 105 - 32, y))
            place = time - 105 -32
            while(place > -32):
                surface.blit(sprites[0], (place, y))
                place -= 32
            place = self.level_with - time + 105
            while(place < self.level_with):
                surface.blit(sprites[0], (place, y))
                place += 32

    def check_death(self, time, pos):
        if (time - 105 + 31 - 8 > pos[0]):
            return True
        elif (self.level_with - time + 105 - 32 + 8 < pos[0] + 31):
            return True
        else:
            return False